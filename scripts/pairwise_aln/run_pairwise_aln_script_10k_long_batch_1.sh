...
genotype_log_dir="/nucleus/projects/saraj/vntr/sources/pangenome_project/logs/logs_genotype_10k_long"
aln_log_dir="/nucleus/projects/saraj/vntr/sources/pangenome_project/logs/logs_pairwise_align_10k_long"
command_output="$aln_log_dir/run_aln_command_output_batch_1.txt"
...
target_genotype="/nucleus/projects/saraj/vntr/sources/pangenome_project/target_vntrs/10k_long.txt"
# Create a directory for the logs and outputs.
mkdir -p $aln_log_dir
# Remove previous output file.
#rm -f $command_output; touch $command_output
for log_file_from_advntr in $(ls -t ${genotype_log_dir}/*.log | head -n 20 | tail -n 10); do
