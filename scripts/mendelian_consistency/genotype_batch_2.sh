...
parent_log_dir="$(pwd)/logs_illumina_wgs_5k_short"
...
# For illumina reads:
for sample_name in $(ls ${illumina_samples_dir} | grep HG | head -n 20 | tail -n 10); do
	pushd $advntr_dir
    ...
    output_vcf="$log_dir/${sample_name}_short_vntr_dataset.vcf"
	...
	for parent_sample_name in $(ls ${illumina_samples_dir}/${sample_name}/parents); do
		...
		# Remove previous output file.
		output_vcf="$log_dir/${parent_sample_name}_short_vntr_dataset.vcf"
        #if [ ! -e $output_vcf ]; then
            /usr/bin/time -v python __main__.py genotype \
            --alignment_file $input_bam_file \
            --working_directory $log_dir \
