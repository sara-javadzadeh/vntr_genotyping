...
log_dir="/nucleus/projects/saraj/vntr/sources/pangenome_project/logs/logs_genotype_disease_all_vntrs_v4_test"
...
target_genotype="/nucleus/projects/saraj/vntr/sources/pangenome_project/target_vntrs/disease_associated_all_vntrs_v4.txt"
...
vids=$(cat $target_genotype | head -n 1)
#vids="983808,983811,983812,983813,983814,983815,983817"

for input_sample_dir in $(ls $samples_dir | grep HG | head -n 1); do
    ...
    for input_bam_file in $(ls $samples_dir/$input_sample_dir/*.bam); do
        ...
        ulimit -n 8000
        /usr/bin/time -v /home/saraj/.local/bin/advntr genotype --alignment_file $input_bam_file --working_directory $log_dir -m $all_vntr_db --pacbio --log_pacbio_reads -vid $vids &>> $output
    done
done
