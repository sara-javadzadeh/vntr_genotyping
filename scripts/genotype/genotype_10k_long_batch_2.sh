...
log_dir="/nucleus/projects/saraj/vntr/sources/pangenome_project/logs/logs_genotype_10k_long"
...
for input_sample_dir in $(cat $samples_dir/sample_ids.txt | tail -n 8 | head -n 4); do
    ...
    for input_bam_file in $(ls $samples_dir/$input_sample_dir/*.bam); do
        ...
        ulimit -n 4096
        /usr/bin/time -v advntr genotype --alignment_file $input_bam_file --working_directory $log_dir -m $long_vntrs --pacbio --log_pacbio_reads &>> $output
    done
done
