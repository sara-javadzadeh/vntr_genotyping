...
parent_log_dir="/nucleus/projects/saraj/vntr/sources/pangenome_project/logs/logs_genotype_giab_hifi_integrated"
...
for sample_name in HG002 HG003 HG004; do
    ...
    ## Genotype the child
    for input_bam_file in $(ls ${samples_dir}/${sample_name}/*.bam); do
        # Short VNTR db
        bam_basename=$(basename $input_bam_file | head -c 12)
        output="$log_dir/short_vntr_db/${bam_basename}_short_vntr_db.txt"
        echo "Genotyping $input_bam_file to output $output"
        ...
        # Long VNTR db
        bam_basename=$(basename $input_bam_file | head -c 12)
        output="$log_dir/long_vntr_db/${bam_basename}_long_vntr_db.txt"
        /usr/bin/time -v python -u __main__.py genotype \
        --alignment_file $input_bam_file \
