...
log_dir="/nucleus/projects/saraj/vntr/sources/pangenome_project/logs/logs_genotype_disease_long_w_advntr_1.5.0"
...
for input_sample_dir in $(ls $samples_dir | grep HG); do
    for input_bam_file in $(ls $samples_dir/$input_sample_dir/*.bam); do
        ...
        /usr/bin/time -v advntr genotype --alignment_file $input_bam_file --working_directory $log_dir -m $long_vntr_db --pacbio --log_pacbio_reads -vid $vids &>> $output
    done
done
